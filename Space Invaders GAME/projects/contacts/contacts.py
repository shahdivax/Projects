...
class Contacts:
    db_filename = 'C:/Users/lenovo/Desktop/Space Invaders GAME/projects/contacts/contacts.db'
    ...
    def create_left_icon(self):
        photo = PhotoImage(file='C:/Users/lenovo/OneDrive/Space Invaders GAME/projects/contacts/icons/logo.gif')
    ...
    def create_scrollbar(self):
        ...
        self.scrollbar.grid(row=6,column=3,columnspan=3,rowspan=10,sticky="sn")
        
    def create_button_buttons(self):
        Button(text='Delet Selected',command=self.on_delet_selected_button_clicked,bg='red',fg='white').grid(row=8,column=0,sticky=W,pady=10,padx=20)
    ...
    def open_modify_window(self):
        name = self.tree.item(self.tree.selection())['text']
        old_no = self.tree.item(self.tree.selection())['values'][1]
        ...
        Entry(self.transient, textvariable=StringVar(
            self.transient, value=old_no), state='readonly').grid(row=1, column=2)
        ...
        Button(self.transient, text='Update Contact', command=lambda: self.update_contacts(
            new_phone_number_entry_widget.get(), old_no, name)).grid(row=3, column=2, sticky=E)
        

        self.transient.mainloop()
        
    def update_contacts(self, newphone, old_phone,name):
        query = 'UPDATE contacts SET number=? WHERE number =? AND name =?'
